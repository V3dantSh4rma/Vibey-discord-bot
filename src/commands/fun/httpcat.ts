...
export default class Httpcat extends Command {
	public category : CommandCategories = "FUN";

	public builder() : Omit<SlashCommandBuilder, "addSubcommand" | "addSubcommandGroup">{
		return new SlashCommandBuilder()
			.setName("httpcat")
			.setDescription("Receive a Cat Picture for every HTTP Status Code.")
	...
	async handle( interaction : CommandInteraction<CacheType>, client : Vibey ) : Promise<void>{
		...
		try {
			if ( statusCode < 100 ) {
				await interaction.followUp("The Number should be between 100-599");
				return;
			}
